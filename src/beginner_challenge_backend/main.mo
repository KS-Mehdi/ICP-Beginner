...
actor {
    ...
    // Récupère les résultats existants de l'utilisateur
    let existingResults = switch (Map.get(userResultsMap, nhash, userId)) {
        case (?found) found;
        case (_) Vector.new<Text>();
    };

    // Ajoute le nouveau résultat à la liste des résultats existants
    Vector.add(existingResults, result);

    // Met à jour la map des résultats avec les nouveaux résultats
    Map.set(userResultsMap, nhash, userId, existingResults);

    // Retourne un résultat contenant l'identifiant de l'utilisateur et la liste mise à jour des résultats
    return #ok({id = userId; results = Vector.toArray(existingResults)});
};
